...
import SmartPlants_Functions as s
import asyncio
...
async def ground_data(request):
    ...
    sensors_data = {
            ...
            "sensor_1_ground": s.sensor_state(sensor_voltage_1),
            "sensor_2_ground": s.sensor_state(sensor_voltage_2),
            "sensor_3_ground": s.sensor_state(sensor_voltage_3),
            "sensor_4_ground": s.sensor_state(sensor_voltage_4),
            }
...
@app.route('/automatics', methods=['POST',"GET"])
async def automatics(request):
    if request.method=="POST":
        data = request.json
        s.turn_on = data['turn_on']
        asyncio.create_task(s.automatics_start(data=data))
        return {"turn_on":s.turn_on}
    if request.method=="GET":
        return {"turn_on":s.turn_on}
